...
def add_cycle_times_gateway_classes(group):
    ...
    for index, row in islice(group.iterrows(), 0, len(group) - 1):
        ...
        if group.loc[index, dataset_manager.activity_col] == "Payment" and group.loc[
                    index + 1, dataset_manager.activity_col] == "Payment":
            group["x41"] = 1
    #
    ...
    if sum(group[dataset_manager.activity_col] == "Insert_Fine_Notification") > 0:
        group["x11"] = 1
    else:
        group["x11"] = 0

    if sum(group[dataset_manager.activity_col] == "Insert_Fine_Notification") > 0 and sum(group[dataset_manager.activity_col] == "Insert_Date_Appeal_to_Prefecture") > 0:
        group["x21"] = 0
    elif sum(group[dataset_manager.activity_col] == "Insert_Fine_Notification") > 0 and sum(group[dataset_manager.activity_col] == "Insert_Date_Appeal_to_Prefecture") == 0:
        group["x21"] = 1

    if sum(group[dataset_manager.activity_col] == "Payment") == 0:
        group["x31"] = 0
    else:
        group["x31"] = 1

    if sum(group[dataset_manager.activity_col] == "Payment") == 1:
        group["x41"] = 0

    if sum(group[dataset_manager.activity_col] == "Send_for_Credit_Collection") > 0:
        group["x51"] = 0
    elif sum(group[dataset_manager.activity_col] == "Send_Appeal_to_Prefecture") > 0:
        group["x51"] = 1
    else:
        group["x51"] = 2

    if sum(group[dataset_manager.activity_col] == "Send_Appeal_to_Prefecture") > 0:
        if sum(group[dataset_manager.activity_col] == "Receive_Result_Appeal_from_Prefecture") > 0:
            group["x61"] = 1
        else:
            group["x61"] = 0
...
#data["x41"] = 0  # some gateways (self-loops) include a path that is *always* taken, i.e. branches are not exclusive
