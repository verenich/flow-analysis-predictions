...
filenames <- list.files()[grep(paste("^validation_calibrated_FA2_(?=.*\\.csv)", sep=''), list.files(), perl=TRUE)]
for (filename in filenames) {
  if(grepl("combined", filename)) {
  this = read.csv(filename, sep=",", header = TRUE)
  this = this[this$metric == "mae",]
...
```{r include=FALSE}
pdf(file="combined-0660.pdf",family="CM Roman",width=12,height=10)
print(p)
dev.off()
embed_fonts("combined-0660.pdf",outfile="combined-0660.pdf")
```
## To-do:
